...
public class JPAPersonManager {
    ...
    public List<Person> findAgeBetween(int minAge, int maxAge) {
        EntityManager em = emf.createEntityManager();
        return em
            .createQuery(
                "select p from Person p where p.age between :minAge and :maxAge",
                Person.class
            )
    ...
    private void verifyPersonExist(Person p) throws UnknownPersonException {
        if (
            this.findByEmail(p.getEmail()) == null
    ...
    public void insertOneTransaction(List<Person> persons)
        ...
        try {
            ...
            throw new PersonAlreadyExistException(
                "La personne existe déjà en base de données"
            );
        }
        em.getTransaction().commit();
    }

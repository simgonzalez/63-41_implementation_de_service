...
public class UpperLimitedPositiveSimpleCounter extends SimpleCounter {
  ...
  public UpperLimitedPositiveSimpleCounter(int initialValue, int maxValue) throws CounterException {
    if (initialValue < 0) {
      throw new CounterException(
          "une valeur plus grande que 0 est attendue, votre valeur est : (" + initialValue + ")");
    }

    if (initialValue > maxValue) {
      throw new CounterException(
          "une valeur initiale plus petite que le max : "
              + maxValue
              + " est attendue, votre valeur est : ("
              + initialValue
              + ")");
    }
    super(initialValue);
  ...
  public void inc() throws CounterException {
    if (this.getValue() + 1 > maxValue) {
      throw new CounterException(
          "La valeur du compteur dépasse la valeur maximale "
              + maxValue
              + "\nValeur avant incrément:"
              + this.getValue());
    }
    super.inc();
  }
  ...
  public void add(int step) throws CounterException {
    if (this.getValue() + step > maxValue || this.getValue() + step < 0) {
      ...
      throw new CounterException(
          "La valeur du compteur dépasse la valeur maximale ou est plus petit que 0"
              + maxValue
              + "\nValeur après ajout du step:"
              + (this.getValue() + step)
              + "\nstep: "
              + step);
    }
